	    				...
	    				<li>
							<div class="thumbnail">
								<div class="caption clearfix">
									<h3><strong>Delivery #{{ $schedule['id'] }}</strong><br/><small>created: {{ $schedule['created_at'] }}</small></h3>
									<ul>
										<li class="clearfix col-lg-12 col-md-12 col-sm-12 col-xs-12">
											<p class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
												<a href="{{ route('delivery_index') }}">Contact Info</a>
												<br/>
												<i>{{ $schedule['contact_name'] }}</i>
												<br/>
												<i>{{ $schedule['contact_number'] }}</i>
											</p>
											<p class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
												<a href="{{ route('address_index') }}">{{ $schedule['address_name'] }}</a>
												<br/>
												<i>{{ $schedule['pickup_address_1'] }}
												</br>{{$schedule['pickup_address_2']}}
												</i>
											</p>
											<p class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
												<a href="{{ route('delivery_index') }}">Pickup Date</a>
												<br/>
												<i>{{ $schedule['pickup_date'] }}</i>
												<br/>
												<i>{{ $schedule['pickup_time'] }}</i>
											</p>
										</li>
										<li class="clearfix col-lg-12 col-md-12 col-sm-12 col-xs-12">
											<p class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
												<a href="{{ route('delivery_index') }}">Dropoff Date</a>
												<br/>
												<i>{{ $schedule['dropoff_date'] }}</i>
												<br/>
												<i>{{ $schedule['dropoff_time'] }}</i>
											</p>
											<p class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
												<a href="{{ route('delivery_index') }}">Special Instructions</a>
												<br/>
												<i>{{ $schedule['special_instructions'] }}</i>
											</p>
										</li>
									...
									<ul class="clearfix col-xs-12 col-sm-12 col-md-12 col-lg-12">
										<li class="pull-left"><a href="{{ route('delivery_delete',$schedule['id']) }}" class="btn btn-danger" role="button">Cancel</a>&nbsp</li>
										<li class="pull-left"><a href="{{ route('delivery_update',$schedule['id']) }}" class="btn btn-default" role="button">Edit</a>&nbsp</li>
									</ul>
								</div>
							</div>
	            		</li>
