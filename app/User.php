    ...
    */
    public static function prepareResults($data) {
        ...
        switch ($special_param) {
            case '!':
                $results = User::searchByInvoiceId($search);
                break;
            case '@':

                $results = User::searchByCustomerMark($search);

                break;
            default: //return regular search
                $results = User::searchByQuery($search);
    ...
    private static function searchByInvoiceId($query){
        ...
        if(count($invoices) == 1){ // Only one invoice returned
            $results = [
                'status'=>true,
                'redirect'=>'InvoicesController@getView',
                'param'=>$query,                        
                'data'=>$invoices,
            ...
            $results = [
                'status'=>true,
                'redirect'=>'InvoicesController@getIndex',
                'param'=>$query,                        
                'data'=>$invoices,
    ...
    private static function searchByCustomerMark($query){
        ...
        if(count($custids) == 1){ // If only one customer is found
            ...
            $results = [
                'status'=>true,
                'redirect'=>'CustomersController@getView',
                'param'=>$query,
                'data'=>$customer,
            ...
            $results = [
                'status'=>false,
                'redirect'=>'CustomersController@getIndex',
                'param'=>$query,
                'data'=>null,
    ...
    private static function searchByQuery($query) {
        $results = [];
        if(is_numeric($search)) { //Is a number so start checking for lengths
            if(strlen($data) > 6) { // Must be a phone number
                $results = User::searchByPhoneNumber($query);

                User::where('contact_phone', 'like', '%'.$query.'%')->get();
            } elseif(strlen($data) == 6) { //must be our invoice
                $invoices = User:
